...
{
    public class PlayerEmoCollision : MonoBehaviour
    {
        ...
        {
            if (MagicCube.GetComponent<StateChange>().state == false && exit == false)
            {
                exit = true;
                player.GetComponent<PlayerController>().EmoNumber += 1;
